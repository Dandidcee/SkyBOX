...
app.get('/api/messages/:conversationId', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await pool.query('SELECT id, conversation_id, external_message_id, reply_to_message_id, direction, type, content as body, media_url, created_at FROM messages WHERE conversation_id = $1 ORDER BY created_at ASC', [conversationId]);
    res.json(result.rows);
  } catch (err) {
    console.error(err);
