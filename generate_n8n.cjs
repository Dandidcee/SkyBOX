...
const promptCode = fs.readFileSync('prompt.txt', 'utf8');
...
const workflow = {
          ...
          "rules": [
            {
              ...
              "conditions": {
                "number": [
                  { "value1": "={{ $json.confidence }}", "operation": "less", "value2": 70 }
                ]
              }
            },
            {
              "intent": "route",
              "output": "Order Closing",
              "conditions": {
                "string": [
                  { "value1": "={{ $json.intent }}", "operation": "equal", "value2": "closing" }
                ]
              }
            }
          ]
        },
        "fallbackOutput": "Lanjut",
        "options": {}
      },
      "name": "Switch Intent & Confidence",
  ...
  "connections": {
    ...
    "Kirim Pesan Teks": {
      "main": [
        [
          { "node": "Switch Intent & Confidence", "type": "main", "index": 0 }
        ]
      ]
    },
